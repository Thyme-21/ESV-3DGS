...
from utils.loss_utils import l1_loss, l1_loss_mask, l2_loss, ssim, loss_photometric,patch_norm_mse_loss,ranking_loss
...
def total_variation_loss(disp: torch.Tensor) -> torch.Tensor:
    """
    计算单通道 disp 的全变差（TV）损失。
    disp: shape = [B, 1, H, W] 或 [B, H, W] 也行。
    """
    # 如果 disp 是 [B, H, W]，可先 unsqueeze(1) 变成 [B, 1, H, W]
    if disp.dim() == 3:
...
def second_order_tv_loss(disp: torch.Tensor) -> torch.Tensor:
    """
    计算二阶TV损失 (对一阶梯度再次做差分).
    disp 形状同上, [B, 1, H, W] 或 [B, H, W].
    """
    ...
    # 第一步：计算一阶梯度
    dx = disp[:, :, :, 1:] - disp[:, :, :, :-1]  # [B,1,H,W-1]
    dy = disp[:, :, 1:, :] - disp[:, :, :-1, :]  # [B,1,H-1,W]

    # 第二步：对 dx, dy 再做一次梯度, 得到二阶差分
    # 只考虑同向差分：dxx, dyy
    dxx = dx[:, :, :, 1:] - dx[:, :, :, :-1]  # [B,1,H,W-2]
    dyy = dy[:, :, 1:, :] - dy[:, :, :-1, :]  # [B,1,H-2,W]

    # 取绝对值并平均
    loss_2nd = dxx.abs().mean() + dyy.abs().mean()
...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # 在训练循环中，调用渲染之前，确保 bg 被定义
        bg = torch.rand((3), device="cuda") if opt.random_background else background

        render_pkg = render(viewpoint_cam, GsDict[f'gs0'], pipe, bg)
        ren_depth = render_pkg["depth"]
        disp = 1.0 / (ren_depth + 1e-8)
        # 如果 disp 的形状不满足 [B, 1, H, W]（例如 ren_depth 可能是 [H, W]），则需要调整：
        if disp.dim() == 2:
            disp = disp.unsqueeze(0).unsqueeze(0)

        if bg_mask is not None:
            # 假设 bg_mask 是 bool 类型的
            fore_mask = ~bg_mask  # 前景区域为背景mask的逻辑非
        else:
            # 如果没有 bg_mask，则默认整个图像都是前景
            fore_mask = torch.ones_like(gt_image[0:1], dtype=torch.bool)

        # loss += 0.1 * patch_norm_mse_loss(image, gt_image, fore_mask, patch_size=16)
        loss += 0.05 * ranking_loss(image, gt_image, patch_size=16)
        ...
        # loss_tv = total_variation_loss(disp)
        # loss_tv_2nd = second_order_tv_loss(disp)
        # lambda_tv = 0.01
        # lambda_tv_2nd = 0.02
        # loss += lambda_tv * loss_tv + lambda_tv_2nd * loss_tv_2nd

        # loss_tv = total_variation_loss(disp)
        # lambda_tv = 0.05
        # loss += lambda_tv * loss_tv
                ...
                for i in range(args.gaussiansN):
                    ...
                    if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                        ...
                        lpm.points_addition(opt.densify_grad_threshold, size_threshold, viewpoint_cam,
                                            current_view_index, sampled_index, image, gt_image)
