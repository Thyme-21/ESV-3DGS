...
class LPM:
    ...
    def find_neighbor_cam(self, viewpoint_cam):
        ...
        # 如果有配对视图，随机选择一个；否则返回 -1
        if len(self.paired_views[camera_index]) > 0:
            random_index = randint(0, len(self.paired_views[camera_index]) - 1)
        else:
            random_index = -1

            # 根据需要，返回处理后的 image 和 gt_image（这里简单返回原始图像）
        image = viewpoint_cam.original_image.cuda()  # 或者你需要预处理后的图像
        gt_image = viewpoint_cam.original_image.cuda()  # 同上

        return camera_index, random_index, image, gt_image
