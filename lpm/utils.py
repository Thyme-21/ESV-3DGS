...
def set_rays_od(cams):
    for id, cam in enumerate(cams):
        ...
        if rayd is not None:
            ...
            proj_mat = cam.projection_matrix.T
            # 加上正则项，避免奇异
            proj_mat_reg = proj_mat + 1e-6 * torch.eye(proj_mat.shape[0], device=proj_mat.device)
            projectinverse = torch.linalg.inv(proj_mat_reg)
